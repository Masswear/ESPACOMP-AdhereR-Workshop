...
```{r, warning=FALSE}
renderPlot({
example_data <- na.omit(medication_events[grepl("^A09A",ATC.CODE) & ID == 8,], cols = "DURATION")
...
example_data <- na.omit(medication_events[grepl("^R03",ATC.CODE) & ID %in% c(11,14) & DURATION != 0,], cols = "DURATION")
...
time_init <- time_to_initiation(presc.data = durcomp.prescribing[grepl("^A09A",ATC.CODE),],
                                disp.data = durcomp.dispensing[grepl("^A09A",ATC.CODE),],
                                ID.colname = "ID",
...
renderPlot({
  ...
  TEs <- compute.treatment.episodes(as.data.frame(example_data),
                                    ...
                                    observation.window.duration=720,
                                    followup.window.duration = 720,
                                    followup.window.duration.unit = "days",
                                    date.format = "%Y-%m-%d")
...
dropdownButton(
  ...
  checkboxInput(inputId = "carry_within1", label = "Carry-over within OW", value = FALSE),
...
renderPlot({
  example_data <- example_data[ID == 11, ]
  example_data$rownr <- 1:nrow(example_data)

   cmaE <- CMA_per_episode(CMA="CMA7", # apply the simple CMA9 to each treatment episode
                        ...
                        event.daily.dose.colname="DAILY.DOSE",
                        medication.class.colname="ATC.CODE",
                        carryover.within.obs.window = input$carry_within1,
