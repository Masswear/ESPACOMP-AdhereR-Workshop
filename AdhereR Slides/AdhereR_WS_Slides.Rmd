...
library(dplyr)
library(kableExtra)
source("data/matching_function.R")

load("data/durcomp.rda")

medication_events <- compute_event_durations(disp.data = durcomp.dispensing, 
...
disp_val <- fread("data/Validation_Data_disp.csv")
presc_val <- fread("data/Validation_Data_presc.csv")
hosp_val <- fread("data/Validation_Data_hosp.csv")
```
...
## Example: Method-related variation in adherence to sibutramine

<img src="figures/hess_example.JPG" style="width: 80%"/>

LM Hess - 2006. https://doi.org/10.1345/aph.1H018.
...
- *Adherence*
    + continuous multiple-interval measures of medication availability (CMA)
- *Initiation*
    + the length of time between the first prescribing event and the first dispensing event
- *Persistence*
    + the length of time with repeated medication events, before discontinuing for a time period longer than a pre-specified permissible gap
- *Implementation*
    + CMA during treatment episodes or observation windows with no treatment gaps longer than a pre-specified period

## ABC Taxonomy for EHD

<figure>
<img src="figures/ABC-taxonomy-EHD.JPG" style="width: 100%"/>
<figcaption>ABC taxonomy figure adapted for EHD</fugcaption>
</figure>
...
```{r, warning=FALSE}
renderPlot({
example_data <- na.omit(medication_events[grepl("^R03AC",ATC.CODE) & ID == 16,], cols = "DURATION")
...
TEs <- compute.treatment.episodes(example_data,
                                  ...
                                  carryover.within.obs.window = FALSE, # carry-over inside the OW
                                  carry.only.for.same.medication = FALSE, # & only for same type
...
cma7 <- CMA7(data=example_data, # use the two selected patients
             ...
             followup.window.duration = 720,
             followup.window.duration.unit = "days",
             observation.window.start=as.Date("2057-01-01", format = "%Y-%m-%d"), # OW start in days since earliest event
             observation.window.duration=365, # OW duration in days
             date.format="%Y-%m-%d"); # date format (mm/dd/yyyy)
...
for(i in 1:nrow(TEs)){
  id = TEs[i, "ID"]
  bottom = head(example_data[ID == id,"rownr"],1)
  top = tail(example_data[ID == id,"rownr"],1)
  start = as.numeric(difftime(TEs[i, "episode.start"],head(TEs[TEs$ID == id, "episode.start"],1), "days"))
  end = as.numeric(difftime(TEs[i, "episode.end"],head(TEs[TEs$ID == id, "episode.start"],1), "days"))

  rect(xleft=start, xright=end, ybottom=bottom-0.45, ytop=top+0.45, border = "blue", lty = "dashed", lwd = 2)
}
...
- The raw data (A),
- A tidy data set (B),
- A code book with all variables and values in the tidy data set,
- A reproducible recipe how to go from A to B.

## Why reproducibility is important

<figure>
<img src="figures/R_Rcorrect-1.png" style="width: 80%"/>
<figcaption>Herndon, T., Ash, M., & Pollin, R. (2014). Cambridge journal of economics, 38(2), 257-279.</fugcaption>
</figure>
...
- coding medication type depending on clinical considerations
- calculate medication event durations (if necessary)
- check plausible values and correcting any deviations
- handle missing data
- see Huang, Yunyu, Jaco Voorham, und Flora M Haaijer-Ruskamp. Journal of Comparative Effectiveness Research 5, Nr. 4 (27. Juni 2016): 345-54. https://doi.org/10.2217/cer-2015-0022.

<!-- ## Select medication events -->
...
<!-- #  -->
<!-- # durcomp.prescribing_A09 <- durcomp.prescribing[grepl("^A09",ATC.CODE),] -->
<!-- # durcomp.dispensing_A09 <- durcomp.dispensing[grepl("^A09",ATC.CODE),] -->
<!-- # ``` -->

## R Basics: structure of analysis (1/2)

- load necessary packages
    + `library(Adherer)`
    + `library(data.table)`
- import data
    + `load("path/to/data.rda")` to load R data objects
    + `fread("path/to/data.csv")` to import csv-files
    + `dbConnect(...)` to connect to a (remote) database

## R Basics: Data structures

>- data.frames or data.tables hold the data similar to an excel spreadsheet
    + each row is an observation
    + each column is a variable

```{r}
str(medication_events)
```

## R Basics: structure of analysis (2/2)

- manipulate data
    + `data.table` syntax: `data[*subset*, *do*, *by*]`
- store results
    + Basic `R` syntax: `object <- function(data, argument)`
    + `save(data, ... , file="path/to/file.rda")` to save R data object
    + `write.csv(data, file = "path.to.file.csv")` to export to csv-file
...
- New AdhereR function to:
    ...
    2. check for prescription changes, hospitalizations, and other treatment interruptions during this period, and
...
dropdownButton(
  ...
  selectInput(inputId = "source", label = "Data source",
              choices = c("prescribing", "dispensing", "hospitalization"), selected = "prescribing"),
  icon = icon("gear"),
  width = "100px"
  )

renderDataTable({
  if(input$source == "prescribing") {
  data <- durcomp.prescribing
} else if(input$source == "dispensing") {
  data <- durcomp.dispensing
} else if(input$source == "hospitalization") {
  data <- durcomp.hospitalisation
}
...
`compute_event_durations(disp.data, presc.data, hosp.data, ...)`
...
dropdownButton(
  ...
  checkboxInput(inputId = "hosp", label = "consider hospitalizations", value = FALSE),
  checkboxInput(inputId = "force.init.presc", label = "force.init.presc", value = FALSE),
  checkboxInput(inputId = "force.presc.renew", label = "force.presc.renew", value = FALSE),
  checkboxInput(inputId = "split.on.dosage.change", label = "split.on.dosage.change", value = FALSE),
  selectInput(inputId = "trt.interruption", label = "trt.interruption",
              choices = c("continue", "discard", "carryover"), selected = "continue"),
  icon = icon("gear"),
  width = "300px"
  )
...
renderPlot({
  ...
  medication_events <- compute_event_durations(disp.data = disp_val,
                                             presc.data = presc_val,
                                             hosp.data = hosp,
                                             ID.colname = "ID",
                                             presc.date.colname = "DATE.PRESC",
                                             disp.date.colname = "DATE.DISP",
                                             ...
                                             total.dose.colname = "TOTAL.DOSE",
                                             presc.daily.dose.colname = "DAILY.DOSE",
                                             presc.duration.colname = "DURATION",
                                             visit.colname = "VISIT",
                                             force.init.presc = input$force.init.presc,
                                             force.presc.renew = input$force.presc.renew,
                                             split.on.dosage.change = input$split.on.dosage.change,
                                             trt.interruption = input$trt.interruption,
                                             suppress.warnings = FALSE,
                                             return.data.table = TRUE)

med_events <- na.omit(medication_events, cols = "DURATION")

class <- data.table(ATC.CODE = unique(disp_val[,ATC.CODE]),
                    class = substr(unique(disp_val[,ATC.CODE]), 1, 1))

med_events <- merge(med_events, class, by = "ATC.CODE")

cma0 <- CMA0(data=as.data.frame(med_events),
             ID.colname="class",
...
TEs <- unique(med_events[,.(class, START.PRESC, END.PRESC, DAILY.DOSE)])
TEs[is.na(END.PRESC),END.PRESC := as.Date("2016-01-01")]

med_events[,rownr := 1:nrow(med_events)]

plot(cma0, col.cats = rainbow, show.legend = FALSE, align.all.patients = FALSE)
for(i in 1:nrow(TEs)){
  class.i = TEs[[i, "class"]]
  bottom = head(med_events[class == class.i,"rownr"],1)
  top = tail(med_events[class == class.i,"rownr"],1)
  start = as.numeric(TEs[i, "START.PRESC"]-head(TEs[TEs$class == class, "START.PRESC"],1))
  end = as.numeric(TEs[i, "END.PRESC"]-head(TEs[TEs$class == class, "START.PRESC"],1))
  offset = min(TEs[["START.PRESC"]], na.rm = TRUE)-min(med_events[["DISP.START"]], na.rm = TRUE)
...
if(!is.null(hosp)){
for(i in 1:nrow(hosp_val)){

  first_event <- min(med_events[["DISP.START"]], na.rm = TRUE)

  bottom = head(med_events[,"rownr"],1)
  top = tail(med_events[,"rownr"],1)
  start = as.numeric(hosp_val[[i, "DATE.IN"]]-first_event)
  end = as.numeric(hosp_val[[i, "DATE.OUT"]]-first_event)
...
example_data <- na.omit(medication_events[grepl("^R03AC",ATC.CODE) & ID %in% c(7,16) & DURATION != 0,], cols = "DURATION")
  ...
  cma0 <- CMA0(data=example_data, # use the two selected patients
             ...
             medication.class.colname="ATC.CODE"); # date format (mm/dd/yyyy)
...
plot_interactive_cma_shiny <- function(data=as.data.frame(na.omit(medication_events, cols = "DURATION")),
                                       cma.class="simple",
                                       ID.colname="ID",
                                       event.date.colname="DATE.DISP",
                                       event.duration.colname="DURATION",
...
- CMA itself makes no difference between persistence/non-persistence
- CMA = implementation only if sample/individual is on treatment
    ...
    + *per episode* CMA first identifies treatment episodes then computes CMA for each

## Overall adherence with CMA7

```{r, echo = TRUE}
cma7 <- CMA7(data=example_data,
             ID.colname="ID",
             event.date.colname="DATE.DISP",
             event.duration.colname="DURATION",
             event.daily.dose.colname="DAILY.DOSE",
...
- Requires prescription and dispensing data for the same follow-up period
- yes/no - availability of a dispensing date (within a period of time after prescription)
- time to dispensing - `time_to_initiation()` function

## Time to initiation

`time_to_initiation(disp.data, presc.data, ...)`

```{r, echo = FALSE}

time_init <- time_to_initiation(presc.data = durcomp.prescribing[grepl("^R03AC",ATC.CODE),],
                                disp.data = durcomp.dispensing[grepl("^R03AC",ATC.CODE),],
                                ID.colname = "ID",
...
renderDataTable(time_init, options = list(pageLength=3))
...
```{r}
renderPlot({
  plot(cma0, align.all.patients=TRUE, show.legend = FALSE)
...
> - `compute.treatment.episodes(data, ...)`
...
dropdownButton(
  ...
  checkboxInput(inputId = "dose_change", label = "Consider dosage change", value = FALSE),
  checkboxInput(inputId = "new_episode", label = "New episode after medication change", value = FALSE),
  selectInput(inputId = "gap_unit", label = "Gap unit", choices = c("days", "percent"), selected = "days"),
...
renderPlot({
  example_data$rownr <- 1:nrow(example_data)
  TEs <- compute.treatment.episodes(as.data.frame(example_data),
                                    ID.colname="ID",
                                    event.date.colname="DATE.DISP",
                                    event.duration.colname="DURATION",
                                    event.daily.dose.colname="DAILY.DOSE",
                                    ...
                                    maximum.permissible.gap.unit = input$gap_unit, # unit for the above (days)
                                    followup.window.start = 0, # 2-years FUW starts at earliest event
                                    followup.window.start.unit = "days",
...
> - CMA per episode
> - `CMA_per_episode(CMA, data, ...)`

```{r, echo = FALSE}

dropdownButton(

  selectInput(inputId = "cma", label = "CMA to apply", choices = c("CMA1", "CMA2", "CMA3", "CMA4", "CMA5", "CMA6", "CMA7", "CMA8", "CMA9"), selected = "CMA7"),
  checkboxInput(inputId = "carry_same1", label = "Carry-over only for same medication", value = FALSE),
  checkboxInput(inputId = "dose_change1", label = "Consider dosage change", value = FALSE),
  checkboxInput(inputId = "new_episode1", label = "New episode after medication change", value = FALSE),
    sliderInput("max_gap1", label = "Maximum permissible gap:",
...
renderPlot({
  example_data <- example_data[ID == 16, ]
  example_data$rownr <- 1:nrow(example_data)

   cmaE <- CMA_per_episode(CMA=input$cma, # apply the simple CMA9 to each treatment episode
                        ...
                        event.daily.dose.colname="DAILY.DOSE",
                        medication.class.colname="ATC.CODE",
                        carry.only.for.same.medication = input$carry_same1,
                        consider.dosage.change = input$dose_change1, # conditions on treatment episodes
                        medication.change.means.new.treatment.episode = input$new_episode1,
                        maximum.permissible.gap = as.numeric(input$max_gap1),
...
> - for time-series, (e.g., GEE), or group-based trajectory models
> - `CMA_sliding_window(CMA.to.apply="CMA9", data, ...)`

```{r, echo = FALSE}
dropdownButton(


  checkboxInput(inputId = "carry_same2", label = "Carry-over only for same medication", value = FALSE),
  checkboxInput(inputId = "dose_change2", label = "Consider dosage change", value = FALSE),
...
cmaW <- CMA_sliding_window(CMA.to.apply="CMA9", # apply the simple CMA9 to each sliding window
                           ...
                           event.daily.dose.colname="DAILY.DOSE",
                           medication.class.colname="ATC.CODE",
                           carry.only.for.same.medication = input$carry_same2,
                           consider.dosage.change = input$dose_change2,
                           followup.window.start=0,
                           observation.window.start=0,
...
- calculation of one single adherence value
- two-step process
	+ create treatment groups
	+ aggregate treatment groups into single adherence value

## Treatment groups

- treatment-switches vs. two-drug regimens
- decide which medications can be used interchangeably
- e.g. based on ATC codes

## Aggregation methods

- CMA for periods with ANY treatment available
- CMA for periods with ALL treatments available
- Average of CMAs for individual treatments, unweighted
- Average of CMAs for individual treatments, weighted by treatment episode (DPPR)
- Dichotomized: CMA for individual treatments larger than cutoff
...
- share the analysis code (and anonymized dataset, if possible)
- use RMarkdown to embed code and R plots in your reports

## Take home messages

- Data Preparation & Exploration are essential for meaningful estimation of adherence
- Meaningful Adherence estimation requires clear operationalization of the reported measure
- AdhereR provides functions to prepare, analyze, and visualize EHD
- AdhereR functions are flexible, transparent, and ensure reproducibility

## Practical session(s)

- interactive online-tutorial with example data (for beginners)
- commented R script to use within R Studio (for advanced R users)
- R-Markdown document with explanations and code examples for reference
