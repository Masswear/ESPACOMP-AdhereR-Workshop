...
In principle the output of compute_event_durations can be used directly for CMA computation (after removing rows with zero or missing durations). However, `AdhereR` offers additional functions `prune_event_durations` and `cover_special_periods` to deal with specific cases. Please refer to the vignette [Data preparation for computing adherence to medication in AdhereR](https://masswear.github.io/Webinar_1_06-19/compute_event_durations.html) for additional information.
