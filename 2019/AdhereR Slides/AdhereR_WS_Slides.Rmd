...
prescription_episodes <- event_durations_list$prescription_episodes
...
```{r, warning=FALSE}
renderPlot({
example_data <- na.omit(event_durations[grepl("^R03AC",ATC.CODE) & ID == 16,], cols = "DURATION")
...
example_data <- na.omit(event_durations[grepl("^R03AC",ATC.CODE) & ID %in% c(7,16) & DURATION != 0,], cols = "DURATION")
...
plot_interactive_cma_shiny <- function(data=as.data.frame(na.omit(event_durations, cols = "DURATION")),
                                       cma.class="simple",
                                       ID.colname="ID",
                                       event.date.colname="DATE.DISP",
                                       event.duration.colname="DURATION",
...
time_init <- time_to_initiation(presc.data = prescription_episodes[grepl("^R03AC",ATC.CODE),],
                                disp.data = event_durations[grepl("^R03AC",ATC.CODE),],
                                ID.colname = "ID",
...
- calculation of one single adherence value: Continuous Aggregated Polypharmacy Score (CAPS)
- Process with 4 components:
    + Grouping of related medication classes into treatment groups
    + Prepare data (adjust observation window, apply carryover, cover special periods, etc.)
...
- Periods with ANY treatment available
- Periods with ALL treatments available
- Average of CMAs for individual treatments
- Average of all treatments available per intervall
- Dichotomized: CMA for individual treatments larger than cutoff

((Examples))

## Function CMA_polypharmacy
