...
We can now merge the three datasets and calculate the duration for each dispensing event, using the `compute_event_durations` function, which links dispensing, prescription, and special periods data. For each dispensing event, it automatically selects the last prescribed dose to calculate supply duration, checks for prescription changes and special periods (such as hospitalisations) during this period, and adjusts the duration accordingly. It requires the following input:

- `disp.data` : A *data.frame* with the dispensing data
- `presc.data` : A *data.frame* with the prescription data
- `special.periods.data` : optional, a *data.frame* with the containing the information about special periods 
